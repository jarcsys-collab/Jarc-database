class AppController {
  ...
  onClick(event) {
    ...
    if (action === "confirm-delete-workspace") { try{this.model.deleteWorkspace(target.dataset.payload);this.view.closeOverlay();this.update();this.view.toast("Workspace deleted");}catch(error){this.view.showMessage("Workspace not deleted",error.message);} }
    ...
    if (action === "move-board") { const choices=this.model.workspaces.filter(w=>w.id!==this.model.currentWorkspaceId); this.view.showQuickChoice("Move board to workspace",choices.map(w=>w.name),"apply-move-board",target.dataset.id); }
    if (action === "apply-move-board") { const ws=this.model.workspaces.find(w=>w.name===target.dataset.value); if(ws){this.model.moveBoardToWorkspace(target.dataset.id,ws.id);this.view.closeOverlay();this.update();this.view.toast("Board moved");} }
  ...
  onSubmit(event) {
    ...
    if (action === "paste-import-form") { try{this.model.importRows(this.parseCsv(data.text));this.view.closeOverlay();this.update();this.view.toast(`${this.model.rows.length} pasted records imported`);}catch(error){this.view.showMessage("Import failed",error.message);} }
    if (action === "workspace-settings-form") { this.model.updateWorkspace(this.model.currentWorkspaceId,data); this.view.showSettings(this.model,"workspace"); this.view.toast("Workspace updated"); }
  }
  onKeydown(event) {
  ...
  async importData(file) { if (!file) return; try { const text=await file.text(); if(file.name.toLowerCase().endsWith(".csv")){this.model.importRows(this.parseCsv(text));this.update();this.view.toast(`${this.model.rows.length} CSV records imported`);return;} const data=JSON.parse(text); if(data.workspaces)this.model.restoreBackup(data); else this.model.importRows(data.rows||data); this.update(); this.view.toast(data.workspaces?"Jarc backup restored":`${this.model.rows.length} records imported`); } catch(error) { this.view.showMessage("Import failed",error.message); } }
