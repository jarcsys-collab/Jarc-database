class BoardModel {
  ...
  readStorage() { try { const current=localStorage.getItem("jarc-database-data"); if(current) return JSON.parse(current)||{}; const legacyKey=["medtek","database",String.fromCharCode(118,57)].join("-"); const legacy=localStorage.getItem(legacyKey); if(legacy){ const parsed=JSON.parse(legacy)||{}; localStorage.setItem("jarc-database-data",JSON.stringify(parsed)); localStorage.removeItem(legacyKey); return parsed; } return {}; } catch { return {}; } }
  save() { localStorage.setItem("jarc-database-data", JSON.stringify({ workspaces: this.workspaces, currentWorkspaceId: this.currentWorkspaceId, currentBoardId: this.currentBoardId, currentView:this.currentView, settings: this.settings, profile: this.profile, notifications: this.notifications, recentBoards: this.recentBoards, members:this.members })); }
  normalizeBoards() {
  ...
  openBoard(boardId, workspaceId = this.currentWorkspaceId) { this.currentWorkspaceId = workspaceId; this.currentBoardId = boardId; this.screen = "board"; this.currentView = saved.currentView || "table"; this.activeSavedViewId = null; this.showArchived=false; this.manualSort=false; this.selected.clear(); this.recentBoards = [{ boardId, workspaceId, openedAt: new Date().toISOString() }, ...this.recentBoards.filter((item) => item.boardId !== boardId || item.workspaceId !== workspaceId)].slice(0,10); this.save(); }
  ...
  createWorkspace({name,color="#0f9489",icon="W",description=""}) {
    ...
    const base=clean.toLowerCase().replace(/[^a-z0-9]+/g,"-").replace(/^-|-$/g,"") || "workspace";
    let id=base, n=2; while(this.workspaces.some((space)=>space.id===id)) id=`${base}-${n++}`;
    const boardId=`board-${Date.now()}`;
    const workspace={id,name:clean,color,icon:(icon||clean[0]||"W").slice(0,2),description,boards:[{id:boardId,name:"Untitled board",icon:"D",records:[],activity:[],groups:["New","Working","Done"],savedViews:[],columns:this.defaultColumns.map((column)=>({...column,visible:true,connection:"",required:column.key==="serial",defaultValue:"",options:[]}))}]};
    this.snapshot("Workspace creation undone"); this.workspaces.push(workspace); this.currentWorkspaceId=id; this.currentBoardId=boardId; this.screen="board"; this.save(); return workspace;
  }


  setView(view) { if(["table","list","kanban","calendar"].includes(view)){ this.currentView=view; this.save(); } }
  ...
  addMember({email,role="Editor"}) { const clean=String(email||"").trim(); if(!clean)return false; if(this.members.some((m)=>m.email.toLowerCase()===clean.toLowerCase()))return false; this.members.push({id:`member-${Date.now()}`,name:clean.split("@")[0],email:clean,role,status:"Invited"}); this.save(); return true; }
  ...
  createBoard(name = "Untitled board") {
    ...
    this.workspace.boards.push({ id, name: name.trim() || "Untitled board", icon: "D", records: [], activity: [{ id: Date.now(), text: "Board created", at: new Date().toISOString() }], groups: ["New", "Working", "Done"], savedViews: [], columns: [{ key: "serial", label: "Record name", type: "text", visible: true, connection: "", required: true, defaultValue: "", options: [] }] });
  ...
  saveView(name) { const clean = name.trim(); if (!clean) return false; const view={ id: Date.now(), name: clean, status: this.status, query: this.query, grouped: this.grouped, sortDirection: this.sortDirection, view:this.currentView, visibleColumns:this.board.columns.filter(c=>c.visible!==false).map(c=>c.key), density:this.settings.density }; this.board.savedViews.push(view); this.activeSavedViewId=view.id; this.save(); return true; }
  applyView(id) { const view = this.board.savedViews.find((item) => item.id === Number(id)); if (!view) return; Object.assign(this, { status: view.status, query: view.query, grouped: view.grouped, sortDirection: view.sortDirection }); if(view.view)this.currentView=view.view; if(view.visibleColumns){this.board.columns.forEach(c=>c.visible=view.visibleColumns.includes(c.key));} if(view.density)this.settings.density=view.density; this.showArchived=false; this.activeSavedViewId=view.id; }
  ...
  createBackup() { return JSON.parse(JSON.stringify({ version: 10, exportedAt: new Date().toISOString(), workspaces: this.workspaces, currentWorkspaceId: this.currentWorkspaceId, currentBoardId: this.currentBoardId, settings: this.settings, profile: this.profile, notifications: this.notifications, recentBoards: this.recentBoards })); }
  restoreBackup(data) { if(!data || !Array.isArray(data.workspaces)) throw new Error("This is not a Jarc backup file."); this.snapshot("Backup restore undone"); this.workspaces=data.workspaces; this.currentWorkspaceId=data.currentWorkspaceId || this.workspaces[0].id; this.currentBoardId=data.currentBoardId || this.workspaces[0].boards[0].id; this.settings={...this.settings,...(data.settings||{})}; this.profile={...this.profile,...(data.profile||{})}; this.notifications=data.notifications||[]; this.recentBoards=data.recentBoards||[]; this.normalizeBoards(); this.save(); }
