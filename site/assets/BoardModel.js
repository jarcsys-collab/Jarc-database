class BoardModel {
  ...
  defaultWorkspaces() {
    return [
      { id: "engineering", name: "Engineering", color: "#2f81f7", boards: [
        ...
        { id: "medtek", name: "Jarc Database", icon: "▧", records: [], activity: [] },
  ...
  createWorkspace({name,color="#0f9489",icon="W",description=""}) {
    ...
    const workspace={id,name:clean,color,icon:(icon||clean[0]||"W").slice(0,2),description,boards:[{id:boardId,name:"Untitled board",icon:"▤",records:[],activity:[],groups:["New","Working","Done"],savedViews:[],columns:this.defaultColumns.map((column)=>({...column,visible:true,connection:"",required:column.key==="serial",defaultValue:"",options:[]}))}]};
